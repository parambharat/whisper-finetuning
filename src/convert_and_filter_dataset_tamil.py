...
def load_data_splits(is_streaming=True, stopping_strategy="all_exhausted"):
    ...
    dataset_dict["openslr_train"] = load_dataset(
        "openslr", "SLR65", split="train", use_auth_token=True, streaming=is_streaming
    )
    ...
    dataset_dict["fleurs_train"] = load_dataset(
        ...
        use_auth_token=True,
        streaming=is_streaming,
    ).rename_columns({"transcription": "sentence"})
    ...
    data_dict["test"] = concatenate_datasets(
        [dataset_dict["common_voice_test"], dataset_dict["fleurs_test"]],
        #      stopping_strategy=stopping_strategy
    )
    return data_dict
