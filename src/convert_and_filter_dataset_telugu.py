...
def load_data_splits():
    ...
    data_dict["openslr_dataset_train"] = load_dataset(
        "openslr", "SLR66", split="train", use_auth_token=True
    )
    data_dict["ucla_dataset_train"] = load_dataset(
        "audiofolder", data_dir="../data/telugu/", drop_labels=True
    )["train"]
    data_dict["fleurs_dataset_train"] = load_dataset(
        "google/fleurs", "te_in", split="train", use_auth_token=True
    ).rename_column("transcription", "sentence")
    data_dict["fleurs_dataset_val"] = load_dataset(
        "google/fleurs", "te_in", split="validation", use_auth_token=True
    ).rename_column("transcription", "sentence")
    data_dict["fleurs_dataset_test"] = load_dataset(
        "google/fleurs", "te_in", split="test", use_auth_token=True
    ).rename_column("transcription", "sentence")
...
def export_audio_and_sentence(example, split):
    ...
    new_path = pathlib.Path(f"../data/filtered_datasets/telugu/{split}/{new_name}")
...
if __name__ == "__main__":
    ...
    exports.to_json(
        "../data/filtered_datasets/telugu/metadata.jsonl", lines=True, orient="records"
    )
